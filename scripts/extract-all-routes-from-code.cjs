...
try {
  ...
  // Extract all <Route ... /> blocks (simple greedy match for leaf routes)
  const routeBlocks = content.match(/<Route[\s\S]*?\/>/g) || [];
  
  const routes = routeBlocks
    .filter(block => !block.includes('<Navigate')) // Skip routes that are redirects
    .map(block => {
      const pathMatch = block.match(/path="([^"]+)"/);
      return pathMatch ? pathMatch[1] : null;
    })
    .filter(path => path && !path.includes(':')) // Remove dynamic routes like /:slug
  ...
  const urls = uniqueRoutes.map(route => `${BASE_URL}${route}`).sort();
