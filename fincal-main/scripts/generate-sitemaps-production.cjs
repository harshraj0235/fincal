...
// Fix duplicate domain in path (moneycal.in/moneycal.in/... -> moneycal.in/...) for indexing
function normalizeSitemapUrl(url) {
  if (!url || typeof url !== 'string') return url;
  return url.replace(/^(https?:\/\/[^/]+)\/moneycal\.in(\/|$)/gi, '$1/');
}
...
function createStandardUrl(url, priority, changefreq, lastmod = TODAY) {
  const clean = normalizeSitemapUrl(url);
  return `  <url>
...
if (fs.existsSync(masterPath)) {
  ...
  allUrls = content.split('\n').map(line => normalizeSitemapUrl(line.trim())).filter(line => line && line.startsWith('http'));
...
// Enhanced categorization (use normalized URLs)
allUrls.forEach(url => {
  const urlPath = url.replace(BASE_URL, '').replace(/^\/moneycal\.in/i, '').toLowerCase();
