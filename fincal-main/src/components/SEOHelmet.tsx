...
import { normalizePath, normalizeCanonicalUrl, ensureNoDuplicateDomain } from '../utils/canonicalUrl';
...
const SEOHelmet: React.FC<SEOHelmetProps> = ({
  ...
  React.useEffect(() => {
    ...
    // Enhanced Canonical URL handling (fix duplicate moneycal.in in path for indexing)
    {
      let canonicalHref = url
        ? (url.startsWith('http') ? url : normalizeCanonicalUrl(url))
        : ensureNoDuplicateDomain(`https://moneycal.in${normalizePath(window.location.pathname)}`);
      canonicalHref = ensureNoDuplicateDomain(canonicalHref);
      canonicalHref = canonicalHref
        .replace(/\/+/g, '/')
        .replace(/\?.*$/, '')
        .replace(/#.*$/, '');
      if (canonicalHref !== 'https://moneycal.in' && canonicalHref !== 'https://moneycal.in/') {
        canonicalHref = canonicalHref.replace(/\/$/, '') || canonicalHref;
      }
    ...
    if (breadcrumbs && breadcrumbs.length > 0) {
      const breadcrumbData = {
        ...
        "itemListElement": breadcrumbs.map((item, index) => ({
          ...
          "position": index + 1,
          "name": item.name,
          "item": ensureNoDuplicateDomain(item.url.startsWith('http') ? item.url : normalizeCanonicalUrl(item.url))
        }))
      };
