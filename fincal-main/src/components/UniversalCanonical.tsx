...
const UniversalCanonical: React.FC<UniversalCanonicalProps> = ({
  ...
  useEffect(() => {
    ...
    // Generate canonical URL (strip duplicate /moneycal.in/ in path for indexing fix)
    const generateCanonicalUrl = (): string => {
      if (customCanonical) {
        return ensureNoDuplicateDomain(normalizeCanonicalUrl(customCanonical));
      }
      let cleanPath = normalizePath(location.pathname);
      if (cleanPath !== '/' && cleanPath.endsWith('/')) cleanPath = cleanPath.slice(0, -1);
      cleanPath = cleanPath.replace(/\?.*$/, '').replace(/#.*$/, '');
      const canonicalUrl = `https://moneycal.in${cleanPath}`;
      return ensureNoDuplicateDomain(canonicalUrl);
    };
