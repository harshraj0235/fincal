...
// Preload only resources that exist (avoid 404s that hurt LCP). Add paths when assets are confirmed.
export const preloadCriticalResources = () => {
  const criticalResources: { href: string; as: 'font' | 'image'; crossOrigin?: string }[] = [
    // Only add paths that exist in public/ to avoid 404s
  ];
