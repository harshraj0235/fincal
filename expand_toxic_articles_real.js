...
function processFile(filePath) {
    ...
    // We want to replace the end of each object array:
    //     ]
    // };
    // With our massive new blocks.
    // The regex `\]\n};` won't work perfectly if there's trailing whitespace.
    
    const parts = content.split(/\]\s*;\s*\n|\n\s*\]\n\};/g);
    
    // Wait, let's just do a simple string replace for `\n    ]\n};`
    // To be safe, we will just find all occurrences of `    ]\n};` and replace them.
