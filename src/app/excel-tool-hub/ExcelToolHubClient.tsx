        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 w-full max-w-5xl">
          {tools.map(tool => (
            <div key={tool.slug} className="bg-white rounded-xl shadow-lg p-6 flex flex-col items-center hover:shadow-2xl transition-shadow">
              ...
              <Link href={`/excel-tools/${tool.slug}`} className="mt-auto bg-blue-600 text-white px-6 py-2 rounded-lg font-semibold shadow hover:bg-blue-700 transition-colors">Open</Link>
            </div>
          ))}
        </div>
      </div>
