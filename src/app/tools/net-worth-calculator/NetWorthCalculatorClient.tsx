...
const NetWorthCalculator: React.FC = () => {
  ...
  const calculateNetWorth = () => {
    ...
    if ((assetBreakdown.find(a => a.category === 'Cash & Savings')?.percentage || 0) > 50) {
      recommendations.push("You have a lot of cash. Consider investing excess funds for better returns.");
    }
    
    if ((liabilityBreakdown.find(l => l.category === 'Credit Cards')?.amount || 0) > 0) {
    ...
    if ((assetBreakdown.find(a => a.category === 'Investments')?.percentage || 0) > 20) score += 20;
    if ((liabilityBreakdown.find(l => l.category === 'Credit Cards')?.amount || 0) === 0) score += 10;
