...
import { Shield, TrendingUp, TrendingDown, PieChart, BarChart3, Calendar, Plus, Edit, Trash2, Filter, Download, Upload, Target, AlertCircle, CheckCircle, DollarSign, CreditCard, PiggyBank, Home, Car, ShoppingBag, Heart, BookOpen, Gamepad2 } from 'lucide-react';
...
const ExpenseTracker: React.FC = () => {
  ...
  useEffect(() => {
    const savedExpenses = (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).getItem('expenseTracker');
    if (savedExpenses) {
  ...
  useEffect(() => {
    (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).setItem('expenseTracker', JSON.stringify(expenses));
    calculateAnalysis();
  }, [expenses]);
