...
const SiloPost: React.FC = () => {
  ...
  if (!data) {
    return (
      <div className="text-center py-20 bg-gray-50 min-h-screen pt-40">
        ...
        <button onClick={() => navigate.push('/')} className="bg-emerald-600 text-white px-6 py-2 rounded-lg hover:bg-emerald-700 transition-colors">
          Back to Home
        </button>
      </div>
    );
  ...
  const leftSidebar = <StickyShareBar title={data.title} url={(typeof window !== 'undefined' ? window.location.href : '')} />;
