...
const ExcelToolPost: React.FC = () => {
  ...
  if (!post) {
    return (
      <div className="text-center py-12">
        ...
        <button onClick={() => navigate.push('/exceltool')} className="btn mt-4">Back to Excel Tool</button>
      </div>
    );
  }
  ...
  return (
    <MasterArticleLayout
      ...
      leftSidebar={<StickyShareBar url={(typeof window !== 'undefined' ? window.location.href : '')} title={post.title} />}
      ...
      {relatedPosts.length > 0 && (
        <div className="mt-20 pt-20 border-t border-slate-100">
          <div className="flex items-center justify-between mb-10">
            ...
            <Link href="/exceltool" className="text-xs font-black text-blue-600 uppercase tracking-widest hover:underline flex items-center gap-1">
              View All Tools <ArrowLeft className="w-3 h-3 rotate-180" />
            </Link>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {relatedPosts.map(rp => (
              <Link key={rp.id} href={`/exceltool/${rp.slug}`} className="group block">
