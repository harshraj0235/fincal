...
const MasterclassLesson: React.FC = () => {
  ...
  if (!lesson) return ((() => { return null; return null; })());

  const content = lessonContent[slug];
  if (!content) return ((() => { return null; return null; })());
    ...
    <div className="min-h-screen bg-slate-50 font-sans">
      ...
      <section className="relative pt-28 pb-16 bg-[#0a0c10] text-white overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto px-4 relative z-10">
          <Link href="/calculators" className="inline-flex items-center gap-2 text-slate-400 hover:text-white text-sm font-bold mb-6 transition-colors">
        ...
        <div className="grid lg:grid-cols-12 gap-8">
          <article className="lg:col-span-8 space-y-8">
            ...
            <div className="flex justify-between items-center pt-8">
              {prev ? (
                <Link href={`/series/corporate-wealth-masterclass/${prev.slug}`} className="flex items-center gap-2 text-sm font-bold text-slate-500 hover:text-indigo-600 transition-colors">
                  <ArrowLeft className="w-4 h-4" /> {prev.title.substring(0, 30)}...
                </Link>
              ) : <div />}
              {next ? (
                <Link href={`/series/corporate-wealth-masterclass/${next.slug}`} className="flex items-center gap-2 px-6 py-3 bg-indigo-600 text-white rounded-2xl font-black text-sm hover:bg-indigo-700 transition-colors">
                  Next: {next.title.substring(0, 25)}... <ArrowRight className="w-4 h-4" />
                </Link>
              ) : (
                <Link href="/calculators" className="flex items-center gap-2 px-6 py-3 bg-emerald-600 text-white rounded-2xl font-black text-sm">
            ...
            <div className="bg-white rounded-3xl border border-slate-200 p-6 sticky top-20">
              ...
              <div className="space-y-2">
                {moduleLessons.map((ml) => (
                  <Link key={ml.slug} href={`/series/corporate-wealth-masterclass/${ml.slug}`}
            ...
            <div className="bg-white rounded-3xl border border-slate-200 p-6">
              ...
              <div className="space-y-2">
                {content.internalLinks.map((link: any, i: number) => (
                  <Link key={i} href={link.to} className="flex items-center justify-between p-3 rounded-xl hover:bg-indigo-50 transition-all group">
