...
const Form16PdfToITRInputExcel: React.FC = () => {
  ...
  useEffect(() => {
    const s = (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).getItem('f16ITRSplitMode');
    if (s === 'auto' || s === 'spaces' || s === 'colon') setSplitMode(s);
  }, []);
  useEffect(() => { (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).setItem('f16ITRSplitMode', splitMode); }, [splitMode]);
