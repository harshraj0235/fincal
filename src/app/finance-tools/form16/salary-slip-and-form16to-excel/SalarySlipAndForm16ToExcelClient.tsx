...
const SalarySlipAndForm16ToExcel: React.FC = () => {
  ...
  useEffect(() => {
    const s = (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).getItem('slipF16Split');
    if (s === 'auto' || s === 'spaces' || s === 'colon') setSplitMode(s);
  }, []);
  useEffect(() => { (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).setItem('slipF16Split', splitMode); }, [splitMode]);
