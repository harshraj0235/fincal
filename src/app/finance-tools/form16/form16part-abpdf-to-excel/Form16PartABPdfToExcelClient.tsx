...
const Form16PartABPdfToExcel: React.FC = () => {
  ...
  useEffect(() => {
    const s = (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).getItem('f16SplitMode');
    if (s === 'auto' || s === 'spaces' || s === 'colon') setSplitMode(s);
    const f = (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).getItem('f16FocusLabels');
    if (f === 'true' || f === 'false') setFocusLabels(f === 'true');
  }, []);
  useEffect(() => { (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).setItem('f16SplitMode', splitMode); }, [splitMode]);
  useEffect(() => { (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).setItem('f16FocusLabels', String(focusLabels)); }, [focusLabels]);
