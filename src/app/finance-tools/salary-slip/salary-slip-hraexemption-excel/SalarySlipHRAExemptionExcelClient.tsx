...
const SalarySlipHRAExemptionExcel: React.FC = () => {
  ...
  useEffect(() => {
    const r = (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).getItem('hraDefaultRent');
    if (r) { const v = parseFloat(r); if (!isNaN(v)) setDefaultRent(v); }
    const m = (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).getItem('hraDefaultMetro');
    if (m === 'true' || m === 'false') setDefaultMetro(m === 'true');
  }, []);
  useEffect(() => { (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).setItem('hraDefaultRent', String(defaultRent)); }, [defaultRent]);
  useEffect(() => { (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).setItem('hraDefaultMetro', String(defaultMetro)); }, [defaultMetro]);
