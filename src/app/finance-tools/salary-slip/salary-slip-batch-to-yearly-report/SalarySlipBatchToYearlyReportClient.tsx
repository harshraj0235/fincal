...
const SalarySlipBatchToYearlyReport: React.FC = () => {
  ...
  useEffect(() => {
    const a = (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).getItem('batchFYApril');
    if (a === 'true' || a === 'false') setFyApril(a === 'true');
    const i = (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).getItem('batchInfer');
    if (i === 'true' || i === 'false') setInferFromFilename(i === 'true');
  }, []);
  useEffect(() => { (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).setItem('batchFYApril', String(fyApril)); }, [fyApril]);
  useEffect(() => { (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).setItem('batchInfer', String(inferFromFilename)); }, [inferFromFilename]);
