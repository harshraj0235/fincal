...
const SalarySlipPdfToExcel: React.FC = () => {
  ...
  useEffect(() => {
    const saved = (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).getItem('salarySlipSplitMode');
    if (saved === 'auto' || saved === 'spaces' || saved === 'colon') {
      setSplitMode(saved);
    }
  }, []);
  useEffect(() => {
    (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).setItem('salarySlipSplitMode', splitMode);
  }, [splitMode]);
