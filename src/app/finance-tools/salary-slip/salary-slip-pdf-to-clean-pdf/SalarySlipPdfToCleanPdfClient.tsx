...
const SalarySlipPdfToCleanPdf: React.FC = () => {
  ...
  const savePreset = () => {
    (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).setItem('cleanPayslipFields', JSON.stringify({ fields, brand, logo }));
  };
  const loadPreset = () => {
    const saved = (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).getItem('cleanPayslipFields');
    if (!saved) return;
    try {
