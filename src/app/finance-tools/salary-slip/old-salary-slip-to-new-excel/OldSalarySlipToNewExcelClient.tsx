...
const OldSalarySlipToNewExcel: React.FC = () => {
  ...
  useEffect(() => {
    const saved = (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).getItem('oldToNewCustomMap');
    if (saved) {
      try {
  ...
  useEffect(() => {
    (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).setItem('oldToNewCustomMap', JSON.stringify(customMap));
  }, [customMap]);
