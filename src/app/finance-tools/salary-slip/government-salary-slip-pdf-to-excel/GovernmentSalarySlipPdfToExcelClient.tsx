...
const GovernmentSalarySlipPdfToExcel: React.FC = () => {
  ...
  useEffect(() => {
    const s = (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).getItem('govSplitMode');
    if (s === 'auto' || s === 'spaces' || s === 'colon') setSplitMode(s);
    const e = (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).getItem('govExportMode');
    if (e === 'xlsx' || e === 'csv') setExportMode(e);
    const f = (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).getItem('govFilterLabels');
    if (f === 'true' || f === 'false') setFilterGovLabels(f === 'true');
  }, []);
  useEffect(() => { (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).setItem('govSplitMode', splitMode); }, [splitMode]);
  useEffect(() => { (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).setItem('govExportMode', exportMode); }, [exportMode]);
  useEffect(() => { (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).setItem('govFilterLabels', String(filterGovLabels)); }, [filterGovLabels]);
