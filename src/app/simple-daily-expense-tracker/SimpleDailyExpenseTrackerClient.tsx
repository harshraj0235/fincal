...
const SimpleDailyExpenseTracker: React.FC = () => {
  ...
  const saveToLocalStorage = () => {
    (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).setItem('expenseTrackerData', JSON.stringify(rows));
    alert('Data saved successfully!');
  };

  const loadFromLocalStorage = () => {
    const saved = (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).getItem('expenseTrackerData');
    if (saved) {
