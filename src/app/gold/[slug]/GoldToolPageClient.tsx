...
const WeightConverter: React.FC = () => {
  const [unit, setUnit] = useState<'gram' | 'tola' | 'ounce' | 'mg' | 'kg'>('gram');
  const [value, setValue] = useState<number>(10);
  const gramValue = useMemo(() => {
    if (!Number.isFinite(value) || value < 0) return 0;
    switch (unit) {
      case 'gram': return value;
      case 'tola': return value * 11.6638038;
      case 'ounce': return value * 31.1034768;
      case 'mg': return value / 1000;
      case 'kg': return value * 1000;
      default: return value;
    }
  }, [unit, value]);
  const tola = useMemo(() => gramValue / 11.6638038, [gramValue]);
  const ounce = useMemo(() => gramValue / 31.1034768, [gramValue]);
  const mg = useMemo(() => gramValue * 1000, [gramValue]);
  const kg = useMemo(() => gramValue / 1000, [gramValue]);
  return (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div>
          ...
          <select value={unit} onChange={(e) => setUnit(e.target.value as any)} className="w-full border rounded px-2 py-1">
            ...
            <option value="ounce">Troy Ounce (oz t)</option>
            <option value="mg">Milligram (mg)</option>
            <option value="kg">Kilogram (kg)</option>
          </select>
        </div>
        <div className="p-4 rounded bg-yellow-50 border">
          <div className="font-semibold">Exact Ratios</div>
          <div className="text-sm text-neutral-700">1 tola = 11.6638038 g</div>
          <div className="text-sm text-neutral-700">1 troy ounce = 31.1034768 g</div>
        </div>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="p-4 rounded bg-white border">
          <div className="text-sm text-neutral-600">Grams (g)</div>
          <div className="text-xl font-bold">{gramValue.toFixed(6)}</div>
        </div>
        <div className="p-4 rounded bg-white border">
          <div className="text-sm text-neutral-600">Tola</div>
          <div className="text-xl font-bold">{tola.toFixed(6)}</div>
        </div>
        <div className="p-4 rounded bg-white border">
          <div className="text-sm text-neutral-600">Troy Ounce (oz t)</div>
          <div className="text-xl font-bold">{ounce.toFixed(6)}</div>
        </div>
        <div className="p-4 rounded bg-white border">
          <div className="text-sm text-neutral-600">Milligram (mg)</div>
          <div className="text-xl font-bold">{mg.toFixed(2)}</div>
        </div>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="p-4 rounded bg-amber-50 border">
          <div className="text-sm text-neutral-600">Kilogram (kg)</div>
          <div className="text-xl font-bold">{kg.toFixed(6)}</div>
        </div>
        <div className="p-4 rounded bg-amber-50 border">
          <div className="text-sm text-neutral-600">Quick examples</div>
          <div className="text-xs text-neutral-700 space-x-2">
            <button onClick={() => { setUnit('gram'); setValue(11.6638038); }} className="underline text-amber-700">1 tola → gram</button>
            <button onClick={() => { setUnit('gram'); setValue(31.1034768); }} className="underline text-amber-700">1 oz t → gram</button>
            <button onClick={() => { setUnit('tola'); setValue(10); }} className="underline text-amber-700">10 tola → gram</button>
          </div>
        </div>
        <div className="p-4 rounded bg-amber-50 border">
          <div className="text-sm text-neutral-600">Copy</div>
          <button
            onClick={() => navigator.clipboard.writeText(`${gramValue.toFixed(6)} g = ${tola.toFixed(6)} tola = ${ounce.toFixed(6)} oz t`)}
            className="mt-2 inline-flex items-center px-3 py-1.5 rounded bg-amber-600 text-white text-sm hover:bg-amber-700"
          >
            Copy Result
          </button>
        </div>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/gold-tools" className="text-amber-700 underline">Back to Gold Tools</Link>
        </div>
      </div>
    );
  }
      ...
      <SEOHelmet
        ...
        calculatorData={{
          ...
          features: (
            ...
            tool.type === 'weightConvert' || tool.type === 'gramToTola' || tool.type === 'gramToOunce' ? ['Gram to tola', 'Gram to troy ounce', 'Reverse conversion', 'Mobile friendly'] :
        ...
        faqData={[
          { question: `How do I use the ${tool.name}?`, answer: 'Enter a value, pick unit (gram or tola), and see instant conversions across tola, troy ounce, mg, and kg.' },
          { question: 'Is 1 tola exactly 11.66 grams?', answer: 'In Indian precious metals context, 1 tola is standardized at 11.6638038 grams. This tool uses the precise ratio for accurate results.' },
          { question: 'Does it support reverse conversion?', answer: 'Yes. Choose “Tola” as the unit and enter value to convert to grams, ounces, mg, and kg.' },
          { question: 'Is this mobile-friendly?', answer: 'Yes. The UI is designed for quick input and conversion on phones, including copy result.' }
        ]}
      />
      <div style={{ minHeight: 0 }} aria-hidden="true"></div>
      <div className="min-h-screen bg-gradient-to-br from-yellow-50 via-white to-amber-50">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <div className="mb-6">
            <Link href={`/gold-tools`} className="flex items-center text-neutral-600 hover:text-neutral-900">
              <ArrowLeft className="h-4 w-4 mr-1" /> Back to Gold Tools
            </Link>
          </div>

          <div className="text-center mb-6" style={{ minHeight: 96 }}>
            <h1 className="text-3xl md:text-4xl font-bold text-gray-800">{tool.name}</h1>
            <p className="text-gray-600">{tool.description}</p>
          </div>

          <div className="mb-8">
            <ResponsiveAd slot="gold-tool-top" />
          </div>
          ...
          >
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 mt-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                <Link href="/gold-tools" className="text-amber-700 hover:text-amber-800 underline">All Gold Tools</Link>
                <Link href="/finance-tools/asset-allocation-tool" className="text-amber-700 hover:text-amber-800 underline">Asset Allocation Tool</Link>
              </div>
            </div>
          </div>
          ...
          <div className="mt-8">
            <ResponsiveAd slot="gold-tool-bottom" />
          </div>
          {/* Long-form SEO content */}
          <div className="mt-10" style={{ contentVisibility: 'auto', containIntrinsicSize: '1200px' }}>
            <ToolArticle
              title={tool.name}
