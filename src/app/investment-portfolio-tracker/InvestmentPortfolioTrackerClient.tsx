...
const InvestmentPortfolioTracker: React.FC = () => {
  ...
  const saveToLocalStorage = () => {
    (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).setItem('portfolioData', JSON.stringify(investments));
    alert('Portfolio data saved successfully!');
  };

  const loadFromLocalStorage = () => {
    const saved = (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).getItem('portfolioData');
    if (saved) {
