...
export const BlogPost: React.FC = () => {
    ...
    return (
      <>
        ...
        <div style={{...styles.pageWrapper, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '2rem'}}>
          ...
          <button 
            onClick={() => navigate.push(isAstroBlog ? '/astro-finance' : '/blog')} 
  ...
  const renderTextWithLinks = (text: string) => {
    ...
    while ((match = combinedRegex.exec(safeText)) !== null) {
      ...
      if (match[1] && match[2]) {
        parts.push(<Link key={match.index} href={match[2]} style={styles.a}>{match[1]}</Link>);
        ...
        if (isExternal) {
          ...
          parts.push(<Link key={match.index} href={match[3]} style={styles.a}>{match[4]}</Link>);
        }
      } else if (match[5]) {
          ...
          <main style={styles.container}>
            <article>
              ...
              <div style={styles.articleContent}>
                <h2 style={styles.h1}>{post.title}</h2>
