...
const MonthlyBudgetPlanner: React.FC = () => {
  ...
  const saveToLocalStorage = () => {
    (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).setItem('budgetData', JSON.stringify(rows));
    alert('Budget data saved successfully!');
  };

  const loadFromLocalStorage = () => {
    const saved = (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).getItem('budgetData');
    if (saved) {
