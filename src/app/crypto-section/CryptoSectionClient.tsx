        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {filteredArticles.map(blog => (
            <div key={blog.id} className="bg-gray-800 rounded-xl shadow-lg p-6 border border-gray-700 hover:border-yellow-400 transition-all">
              ...
              <Link href={`/crypto/${blog.slug}`} className="inline-block mt-2 px-6 py-2 bg-yellow-400 text-black font-semibold rounded-full shadow hover:bg-yellow-300 transition-all">पढ़ें</Link>
            </div>
          ))}
        </div>
        {/* Crypto Tool */}
