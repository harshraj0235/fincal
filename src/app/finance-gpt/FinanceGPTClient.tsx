...
function loadChats(): SavedChat[] {
  try {
    const data = (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).getItem(STORAGE_KEY);
...
function saveChats(chats: SavedChat[]) {
  try {
    (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).setItem(STORAGE_KEY, JSON.stringify(chats.slice(0, 50))); // Keep max 50 chats
...
const FinanceGPT: React.FC = () => {
  ...
  const [isDarkMode, setIsDarkMode] = useState(() => {
    return (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).getItem('moneycal_dark_mode') === 'true';
  });
  ...
  useEffect(() => {
    if (isDarkMode) {
      ...
      (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).setItem('moneycal_dark_mode', 'true');
    } else {
      document.documentElement.classList.remove('dark');
      (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).setItem('moneycal_dark_mode', 'false');
    }
  }, [isDarkMode]);
  ...
  useEffect(() => {
    ...
    setSavedChats(prev => {
      ...
      if (activeChatId) {
        ...
        const newId = activeChatId || crypto.randomUUID();
  ...
  const handleSubmit = useCallback(async (queryOverride?: string) => {
    ...
    // Give it a UUID if it doesn't have one and navigate to shareable URL
    if (!activeChatId && !chatId) {
      const newId = crypto.randomUUID();
      setActiveChatId(newId);
      navigate.push(`/ask/${newId}`, { replace: true });
    }
        ...
        <div className={`fgpt-sidebar ${sidebarOpen ? 'fgpt-sidebar-open' : ''}`}>
          ...
          <div className="fgpt-sidebar-trending">
            ...
            {discoverArticles.slice(0, 3).map(article => (
              <Link key={article.id} href={`/discover/${article.slug}`} className="fgpt-sidebar-trend-link">
                {article.title}
              </Link>
            ))}
          </div>
            ...
            <div className="fgpt-hero">
              ...
              <div className="fgpt-trending">
                <div className="fgpt-trending-header">
                  ...
                  <Link href="/discover" className="fgpt-trending-see-all">सभी देखें →</Link>
                </div>
                ...
                {[...discoverArticles]
                  ...
                  .slice(0, 4)
                  .map(article => (
                    <Link key={article.id} href={`/discover/${article.slug}`} className="fgpt-trending-card">
            ...
            <section className="fgpt-seo-section" aria-label="MoneyCal Features & FAQ">
              ...
              <div className="fgpt-seo-tools">
                ...
                <div className="fgpt-seo-links">
                  <Link href="/calculators/sip-calculator" className="fgpt-seo-link">SIP Calculator</Link>
                  <Link href="/calculators/emi-calculator" className="fgpt-seo-link">EMI Calculator</Link>
                  <Link href="/calculators/income-tax-calculator" className="fgpt-seo-link">Income Tax Calculator 2025-26</Link>
                  <Link href="/calculators/mutual-fund-calculator" className="fgpt-seo-link">Mutual Fund Calculator</Link>
                  <Link href="/calculators/fd-calculator" className="fgpt-seo-link">FD Calculator</Link>
                  <Link href="/calculators/rd-calculator" className="fgpt-seo-link">RD Calculator</Link>
                  <Link href="/calculators/ppf-calculator" className="fgpt-seo-link">PPF Calculator</Link>
                  <Link href="/calculators/nps-calculator" className="fgpt-seo-link">NPS Calculator</Link>
                  <Link href="/calculators/home-loan-emi-calculator" className="fgpt-seo-link">Home Loan EMI Calculator</Link>
                  <Link href="/calculators/car-loan-emi-calculator" className="fgpt-seo-link">Car Loan EMI Calculator</Link>
                  <Link href="/calculators/lumpsum-calculator" className="fgpt-seo-link">Lumpsum Calculator</Link>
                  <Link href="/calculators/step-up-sip-calculator" className="fgpt-seo-link">Step-up SIP Calculator</Link>
                  <Link href="/calculators/swp-calculator" className="fgpt-seo-link">SWP Calculator</Link>
                  <Link href="/calculators/cagr-calculator" className="fgpt-seo-link">CAGR Calculator</Link>
                  <Link href="/calculators/gratuity-calculator" className="fgpt-seo-link">Gratuity Calculator</Link>
                  <Link href="/calculators/hra-calculator" className="fgpt-seo-link">HRA Calculator</Link>
                  <Link href="/gold-rate" className="fgpt-seo-link">Gold Rate Today</Link>
                  <Link href="/silver-rate" className="fgpt-seo-link">Silver Rate Today</Link>
                  <Link href="/ipo" className="fgpt-seo-link">IPO Dashboard 2026</Link>
                  <Link href="/calculators" className="fgpt-seo-link">All Calculators →</Link>
                </div>
              </div>
