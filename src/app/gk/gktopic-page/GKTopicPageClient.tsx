...
const GKTopicPage: React.FC = () => {
  ...
  if (!topicData) return ((() => { return null; return null; })());
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-indigo-50/30">
      ...
      <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-xl border-b border-gray-100 shadow-sm">
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
          <Link href={`/gk/category/${category.slug}`} className="flex items-center gap-2 text-gray-600 hover:text-indigo-600 transition-colors">
      ...
      <main className="max-w-4xl mx-auto px-4 py-6 sm:py-8">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-2 text-sm text-gray-500 mb-3">
            <Link href="/gk" className="hover:text-indigo-600">GK</Link>
            <span>/</span>
            <Link href={`/gk/category/${category.slug}`} className="hover:text-indigo-600">{category.name}</Link>
          </div>
        ...
        {stats.attempted === allMCQs.length && allMCQs.length > 0 && (
          <div className="mt-10 bg-gradient-to-r from-indigo-600 to-purple-600 rounded-2xl p-8 text-white text-center shadow-2xl">
            ...
            <div className="flex justify-center gap-4">
              ...
              <Link href="/gk" className="px-6 py-3 border-2 border-white/50 text-white rounded-xl font-semibold hover:bg-white/10 transition-all">
                और Topics देखें
              </Link>
            </div>
          </div>
