...
const GSTRateImpactAnalyzer: React.FC = () => {
  ...
  const [passThroughPercent, setPassThroughPercent] = useState<number>(100);
  ...
  const analysis = useMemo(() => {
    ...
    const passedToCustomer = (priceImpact * (Number(passThroughPercent) || 0)) / 100;
    const absorbedByBusiness = priceImpact - passedToCustomer;
    const marginValueOld = (oldPriceToCustomer * (Number(marginPercent) || 0)) / 100;
    const adjustedNewPrice = oldPriceToCustomer + passedToCustomer;
    const marginValueNew = (adjustedNewPrice * (Number(marginPercent) || 0)) / 100;
    const marginImpact = marginValueNew - marginValueOld;
    return { oldTax, newTax, oldPriceToCustomer, newPriceToCustomer, priceImpact, passedToCustomer, absorbedByBusiness, marginValueOld, marginValueNew, marginImpact };
  }, [basePrice, oldRate, newRate, marginPercent, passThroughPercent]);
    ...
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br from-amber-50 via-white to-yellow-50">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <div className="mb-6">
            <button onClick={() => navigate.back()} className="flex items-center text-neutral-600 hover:text-neutral-900">
          ...
          <div ref={resultsRef} className="bg-white rounded-xl shadow-lg p-6 border border-gray-200">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Tax Change Passed to Customer (%)</label>
                <input type="number" min={0} max={100} value={passThroughPercent} onChange={(e) => setPassThroughPercent(Number(e.target.value))} className="w-full px-3 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-amber-500 focus:border-amber-500" />
              </div>
            </div>
            ...
            <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="p-4 rounded-lg bg-cyan-50 border border-cyan-100">
                <div className="text-xs text-cyan-700">Passed to Customer</div>
                <div className="text-2xl font-bold text-cyan-800">₹{analysis.passedToCustomer.toLocaleString(undefined, { maximumFractionDigits: 2 })}</div>
              </div>
              <div className="p-4 rounded-lg bg-orange-50 border border-orange-100">
                <div className="text-xs text-orange-700">Absorbed by Business</div>
                <div className="text-2xl font-bold text-orange-800">₹{analysis.absorbedByBusiness.toLocaleString(undefined, { maximumFractionDigits: 2 })}</div>
              </div>
            </div>
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mt-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                <Link href="/tools/gst-liability-calculator" className="text-blue-600 hover:text-blue-800 underline">GST Liability Calculator</Link>
                <Link href="/tools/gst-turnover-tracker" className="text-blue-600 hover:text-blue-800 underline">GST Annual Turnover Tracker</Link>
                <Link href="/tools/gst-eway-distance-calculator" className="text-blue-600 hover:text-blue-800 underline">GST E-Way Bill Distance Calculator</Link>
                <Link href="/tools/gst-einvoice-qr-validator" className="text-blue-600 hover:text-blue-800 underline">GST E-Invoice QR Validator</Link>
              </div>
            </div>
          </div>
          ...
          <section className="mt-10 bg-white rounded-xl border border-gray-200 shadow-sm p-6 prose prose-amber max-w-none">
            <h2>GST Rate Impact Analyzer for Indian Pricing Decisions</h2>
            <p>
              This tool helps businesses simulate what happens when GST rate changes from one slab
              to another. Instead of only checking tax difference, the analyzer also shows how much
              impact is passed to customers and how much is absorbed internally by business margin.
            </p>
            <p>
              Many pricing teams struggle when rates move from 12% to 18% or 5% to 12%. If the full
              impact is passed to the market, customer price may jump and demand can soften in
              sensitive categories. If the entire impact is absorbed by business, margins can shrink
              quickly. This analyzer gives a balanced planning view by introducing pass-through
              percentage as a practical decision lever.
            </p>
            <h3>How to interpret output</h3>
            <p>
              Old and new tax amounts show compliance impact. Old and new customer prices show retail
              effect. Passed-to-customer and absorbed-by-business outputs help management select a
              realistic strategy. Margin impact then tells whether your current gross margin remains
              healthy after the chosen pass-through policy.
            </p>
            <p>
              This makes the page useful for long-tail intent such as <strong>GST price impact
              calculator India</strong>, <strong>GST margin impact tool</strong>, and <strong>tax
              rate pass-through calculator</strong>. If you run seasonal promotions, repeat the
              analysis with multiple base prices so your sales and finance teams can align discount
              strategy with tax reality.
            </p>
          </section>
        </div>
      </div>
