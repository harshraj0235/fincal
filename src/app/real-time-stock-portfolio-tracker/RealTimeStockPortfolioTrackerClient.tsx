...
const RealTimeStockPortfolioTracker: React.FC = () => {
  ...
  useEffect(() => {
    const savedPortfolio = (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).getItem('stockPortfolio');
    if (savedPortfolio) {
  ...
  useEffect(() => {
    (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).setItem('stockPortfolio', JSON.stringify(portfolio));
  }, [portfolio]);
