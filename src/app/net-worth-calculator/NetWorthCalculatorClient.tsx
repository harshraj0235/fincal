...
const NetWorthCalculator: React.FC = () => {
  ...
  const saveToLocalStorage = () => {
    (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).setItem('netWorthData', JSON.stringify({ assets, liabilities }));
    alert('Net worth data saved successfully!');
  };

  const loadFromLocalStorage = () => {
    const saved = (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).getItem('netWorthData');
    if (saved) {
