...
export async function fetchLiveRates(citySlug: string = 'mumbai'): Promise<CityRates> {
    // 1. Check Cache
    const cached = (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).getItem(`${CACHE_KEY}_${citySlug}`);
    if (cached) {
    ...
    try {
        ...
        // 3. Save Cache
        (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).setItem(`${CACHE_KEY}_${citySlug}`, JSON.stringify({ data, timestamp: Date.now() }));
        return data;
...
export async function fetchHistoricalRates(citySlug: string, days: number = 10): Promise<HistoricalRate[]> {
    ...
    const cached = (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).getItem(cacheKey);
    if (cached) {
        const { data, timestamp } = JSON.parse(cached);
    ...
    (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).setItem(cacheKey, JSON.stringify({ data: history, timestamp: Date.now() }));
    return history;
}
