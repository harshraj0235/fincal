...
interface ExistingBlogPost {
  ...
  categories?: string[];
  keywords?: string[];
  content?: Array<{ content?: string; items?: string[] }>;
  date?: string;
}
...
export function calculateReadingTime(content: Array<{ content?: string; items?: string[] }> = []): number {
...
export function generateSchema(post: ExistingBlogPost & { focusKeyword: string }): Record<string, unknown> {
...
export function generateOpenGraph(post: ExistingBlogPost & { focusKeyword: string }): Record<string, unknown> {
  const baseUrl = 'https://moneycal.in';
  
  return {
    title: `${post.title} | MoneyCal.in`,
    description: post.metaDescription || post.excerpt || '',
