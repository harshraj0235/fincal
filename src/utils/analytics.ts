...
const GA_MEASUREMENT_ID = 'G-JP39RFG4WC'; // Replace with your actual GA4 measurement ID
...
export const initGoogleAnalytics = () => {
  ...
  if (import.meta.env.DEV) console.log('Google Analytics 4 initialized');
};
...
export const trackEvent = (eventName: string, parameters: Record<string, unknown> = {}) => {
  if (typeof window.gtag !== 'undefined') {
    window.gtag('event', eventName, {
      event_category: typeof parameters.category === 'string' ? parameters.category : 'engagement',
      event_label: typeof parameters.label === 'string' ? parameters.label : '',
