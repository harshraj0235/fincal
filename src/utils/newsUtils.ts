...
export const getNews = async (): Promise<NewsItem[]> => {
  try {
    // Check cache first
    const cachedNews = (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).getItem(CACHE_KEY);
    const cachedTime = (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).getItem(CACHE_TIME_KEY);
    ...
    // Cache the new data
    (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).setItem(CACHE_KEY, JSON.stringify(news));
    (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).setItem(CACHE_TIME_KEY, Date.now().toString());
...
export const clearNewsCache = (): void => {
  (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).removeItem(CACHE_KEY);
  (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).removeItem(CACHE_TIME_KEY);
};
