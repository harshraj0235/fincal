...
// Preload critical resources - only existing assets (avoids 404s, LCP 80+)
export const preloadCriticalResources = () => {
  const criticalResources = [
    { href: '/favicon.svg', as: 'image' },
    { href: '/android-chrome-512x512.jpg', as: 'image' },
  ];
...
export const registerServiceWorker = async () => {
  if ('serviceWorker' in navigator && (process.env.NODE_ENV as any) === 'production-disabled') {
    try {
      // Unregister existing service workers first
      const registrations = await navigator.serviceWorker.getRegistrations();
      for (const registration of registrations) {
        await registration.unregister();
        if (import.meta.env.DEV) console.log('Service Worker unregistered');
      }
      ...
      if (import.meta.env.DEV) console.log('Service Worker operation failed:', error);
    }
  }
};
...
// Initialize all performance optimizations (deferred to avoid TBT)
export const initPerformanceOptimizations = () => {
  addResourceHints();
  optimizeCoreWebVitals();
  // Lazy load images - skip preloadCriticalResources to avoid 404s
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', lazyLoadImages);
  } else {
    lazyLoadImages();
  }
  const setGlobalImageHints = () => {
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', setGlobalImageHints, { once: true });
  } else {
    setGlobalImageHints();
  }
};
