...
const FinanceGPTResponseRenderer: React.FC<FinanceGPTResponseRendererProps> = ({ text, sources = [] }) => {
  ...
  const handleFeedback = (type: "up" | "down") => {
    ...
    try {
      const stored = JSON.parse((typeof window !== "undefined" ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).getItem("moneycal_feedback") || "[]");
      stored.push({
        type,
      ...
      (typeof window !== "undefined" ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).setItem("moneycal_feedback", JSON.stringify(stored.slice(-200)));
  ...
  const markdownToHtml = (md: string): string => {
    ...
    html = html.replace(/\[([^\]]+)\]\(([^)]+)\)/g, (_m, linkText, url) => {
      ...
      if (isInternal) {
        ...
        return `<a "${cleanUrl}" class="gpt-link gpt-link-internal">${linkText}</a>`;
      }
      return `<a "${url}" target="_blank" rel="noopener noreferrer" class="gpt-link">${linkText} ↗</a>`;
    });
    ...
    html = html.replace(/(?<!"|">)(https?:\/\/[^\s<"]+)/g, (url) => {
      ...
      if (isInternal) {
        const cleanUrl = url.replace(/https?:\/\/(www\.)?moneycal\.in/, "");
        return `<a "${cleanUrl}" class="gpt-link gpt-link-internal">🔗 ${displayUrl}</a>`;
      }
      return `<a "${url}" target="_blank" rel="noopener noreferrer" class="gpt-link">🔗 ${displayUrl}… ↗</a>`;
    });
