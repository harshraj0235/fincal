...
export const ArticleToolbar: React.FC<ArticleToolbarProps> = ({ articleTitle }) => {
  ...
  const handleBookmark = () => {
    ...
    const bookmarks = JSON.parse((typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).getItem('newsBookmarks') || '[]');
    if (!bookmarked) {
      bookmarks.push({ title: articleTitle, url: window.location.pathname, date: new Date().toISOString() });
      (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).setItem('newsBookmarks', JSON.stringify(bookmarks));
    } else {
      const filtered = bookmarks.filter((b: any) => b.url !== window.location.pathname);
      (typeof window !== 'undefined' ? window.localStorage : { getItem: () => null, setItem: () => {}, removeItem: () => {} }).setItem('newsBookmarks', JSON.stringify(filtered));
    }
  };
