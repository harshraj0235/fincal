...
import { Car, IndianRupee, TrendingUp, DollarSign } from 'lucide-react';
            ...
            <div className="bg-white rounded-2xl shadow-2xl p-8">
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <input type="number" aria-label="Car on-road price in rupees" value={carPrice} onChange={(e) => setCarPrice(Number(e.target.value))} className="w-full px-4 py-3 border-2 rounded-xl text-lg font-bold" />
                  <input type="range" aria-label="Adjust car price" min="300000" max="5000000" step="50000" value={carPrice} onChange={(e) => setCarPrice(Number(e.target.value))} className="w-full mt-2" />
                </div>

                <div>
                  <label className="block text-sm font-semibold mb-2">Down Payment</label>
                  <input type="number" aria-label="Down payment amount" value={downPayment} onChange={(e) => setDownPayment(Number(e.target.value))} className="w-full px-4 py-3 border-2 rounded-xl text-lg font-bold" />
                  <input type="range" aria-label="Adjust down payment" min="0" max={carPrice * 0.5} step="10000" value={downPayment} onChange={(e) => setDownPayment(Number(e.target.value))} className="w-full mt-2" />
                ...
                <div>
                  ...
                  <input type="number" aria-label="Annual interest rate" value={interestRate} onChange={(e) => setInterestRate(Number(e.target.value))} step="0.1" className="w-full px-4 py-3 border-2 rounded-xl text-lg font-bold" />
                </div>

                <div>
                  <label className="block text-sm font-semibold mb-2">Loan Tenure (Years)</label>
                  <input type="number" aria-label="Loan tenure in years" value={loanTenure} onChange={(e) => setLoanTenure(Number(e.target.value))} className="w-full px-4 py-3 border-2 rounded-xl text-lg font-bold" />
                </div>
              </div>
            </div>
