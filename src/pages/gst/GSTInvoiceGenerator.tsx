...
const GSTInvoiceGenerator: React.FC = () => {
  ...
  const [items, setItems] = useState<ItemRow[]>([
    { description: 'Product/Service', hsn: '', qty: 1, rate: 0, gstRate: 18 }
  ]);

  const totals = useMemo(() => {
    const subTotal = items.reduce((sum, i) => sum + i.qty * i.rate, 0);
    const tax = items.reduce((sum, i) => sum + (i.qty * i.rate * (i.gstRate || 0)) / 100, 0);
  ...
  }, [items, invoiceMeta.isIGST]);

  const addItem = () => setItems(prev => [...prev, { description: '', hsn: '', qty: 1, rate: 0, gstRate: 18 }]);
  ...
  return (
    <>
      <SEOHelmet
        title="Free GST Invoice Generator (PDF) | MoneyCal.in"
        description="Create professional GST invoices with CGST/SGST/IGST split. Add items, auto-calc totals, and download as PDF. No login required."
        keywords="gst invoice generator, gst bill maker, gst invoice pdf, free invoice india"
        url="/gst-tools/gst-invoice-generator"
        type="website"
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                ...
                <tbody>
                  {items.map((row, idx) => (
                    <tr key={idx} className="border-t">
                      ...
                      <td className="px-3 py-2 text-right">₹{(row.qty * row.rate).toLocaleString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mt-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
              <RouterLink to="/gst-tools/gst-einvoice-qr-validator" className="text-blue-600 hover:text-blue-800 underline">E-Invoice QR Validator</RouterLink>
