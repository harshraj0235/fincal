...
export const BlogPost: React.FC = () => {
  ...
  const related = relatedPosts.length ? relatedPosts : featuredPosts.filter(p => p.slug !== post.slug).slice(0, 3);
    ...
    <div className="min-h-screen flex flex-col md:flex-row bg-neutral-50">
      ...
      <main className="flex-1 min-w-0 flex flex-col">
        ...
        <div className="sticky top-0 z-20 bg-white/95 backdrop-blur-md shadow-sm border-b border-neutral-100 mb-4">
          <div className="overflow-x-auto scrollbar-hide py-1 px-2 flex gap-2 items-center">
        ...
        <div className="max-w-4xl mx-auto px-2 sm:px-6 lg:px-8 py-6 sm:py-12 w-full">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-12">
            ...
            {/* Related Posts (bottom on mobile, sidebar on desktop) */}
            <div className="lg:col-span-1 mt-8 lg:mt-0">
              <h3 className="text-lg font-semibold text-neutral-900 mb-4">Related Posts</h3>
              <div className="space-y-4">
                {related.map(rp => (
                  <Link key={rp.id} to={`/blog/${rp.slug}`} className="block bg-white rounded-xl shadow hover:shadow-lg transition-shadow border border-neutral-100 p-4">
                    <div className="flex items-center gap-3">
                      <img src={rp.coverImage} alt={rp.title} className="h-16 w-16 object-cover rounded-lg" />
                      <div>
                        <h4 className="text-base font-semibold text-neutral-900 line-clamp-2">{rp.title}</h4>
                        <p className="text-xs text-neutral-500 line-clamp-2">{rp.excerpt}</p>
                      </div>
                    </div>
                  </Link>
                ))}
