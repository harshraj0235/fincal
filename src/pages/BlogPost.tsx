...
export const BlogPost: React.FC = () => {
  ...
  // const relatedPosts = getRelatedPosts(slug || '', 3); // No longer used
  ...
  // Related posts: show all except current
  const allRelatedPosts = allBlogPosts.filter(p => p.slug !== post.slug);
  ...
  // TypeScript guard: post is always defined after this point
    ...
    <div className="min-h-screen flex flex-col md:flex-row bg-neutral-50">
      ...
      <aside className="hidden md:block w-64 flex-shrink-0 border-r border-neutral-200 bg-white/95 backdrop-blur-md sticky top-0 h-screen z-10">
        <div className="p-6">
            ...
            {categories.map(category => (
              <li key={category}>
                <button
                  ...
                  className="flex items-center w-full px-3 py-2 rounded-lg text-left font-medium transition-colors hover:bg-neutral-100 text-neutral-700"
                >
          ...
          <h2 className="text-lg font-bold mt-8 mb-4">Related Posts</h2>
          <ul className="space-y-2">
            {allRelatedPosts.map(rp => (
              <li key={rp.id}>
                <Link to={`/blog/${rp.slug}`} className="block px-3 py-2 rounded-lg hover:bg-primary-50 text-primary-700 font-medium truncate">
                  {rp.title}
                </Link>
              </li>
            ))}
      ...
      <main className="flex-1 min-w-0 flex flex-col">
        ...
        <div className="sticky top-0 z-20 bg-white/95 backdrop-blur-md shadow mb-2">
          <div className="overflow-x-auto scrollbar-hide py-1 px-2 flex gap-2">
        ...
        <div className="max-w-4xl mx-auto px-2 sm:px-6 lg:px-8 py-6 sm:py-12 w-full">
          ...
          <article className="bg-white rounded-2xl shadow-lg p-4 sm:p-8 mb-8">
            ...
            <div className="flex gap-4 overflow-x-auto scrollbar-hide pb-2 -mx-2 px-2">
              {allRelatedPosts.map(rp => (
                <Link
                  key={rp.id}
                  to={`/blog/${rp.slug}`}
                  className="min-w-[80vw] max-w-xs bg-gradient-to-br from-primary-50 to-white rounded-xl shadow-lg overflow-hidden flex-shrink-0 border border-primary-100"
                >
                  ...
                  <div className="p-3">
                    <h3 className="text-base font-semibold text-neutral-900 mb-1 line-clamp-2">{rp.title}</h3>
                    <p className="text-neutral-600 text-xs line-clamp-2">{rp.excerpt}</p>
                  </div>
                </Link>
              ))}
            </div>
        ...
        <nav className="md:hidden fixed bottom-0 left-0 right-0 z-30 bg-white border-t border-neutral-200 flex justify-between items-center px-1 py-1 shadow-lg">
          {categories.map(category => (
            ...
            >
              {categoryIcons[category] || <Tag className="h-5 w-5" />}
              <span className="text-xs mt-0.5 line-clamp-1">{category}</span>
            </button>
          ))}
        </nav>
      </main>
