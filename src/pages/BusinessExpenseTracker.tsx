...
const initialRow = { expenseName: '', category: '', date: '', amount: '', notes: '' };

const BusinessExpenseTracker: React.FC = () => {
  ...
  const handleExcel = useCallback(() => {
    ...
    window.XLSX.utils.book_append_sheet(wb, ws, 'BusinessExpenses');
    window.XLSX.writeFile(wb, 'business-expense-tracker.xlsx');
  }, [rows]);
  ...
  const handlePDF = useCallback(() => {
    ...
    doc.text('Business Expense Tracker', 10, 10);
    let y = 20;
    rows.forEach((row, idx) => {
      doc.text(`${idx+1}. ${row.expenseName} | ${row.category} | ${row.date} | ${row.amount} | ${row.notes}`, 10, y);
      y += 10;
    });
    doc.save('business-expense-tracker.pdf');
  }, [rows]);

  // HTML download
  const handleHTML = useCallback(() => {
    const html = `<!DOCTYPE html><html><head><title>Business Expense Tracker</title></head><body>${document.getElementById('business-expense-table')?.outerHTML}</body></html>`;
    const blob = new Blob([html], { type: 'text/html' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'business-expense-tracker.html';
    a.click();
  ...
  return (
    <div className="max-w-3xl mx-auto p-4 bg-white rounded-xl shadow-lg mt-6 mb-8">
      <h1 className="text-2xl font-bold mb-4 text-blue-700">Business Expense Tracker</h1>
      <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-5 gap-2 mb-4">
        <input type="text" name="expenseName" placeholder="Expense Name" className="input" value={formData.expenseName} onChange={handleChange} required />
        <input type="text" name="category" placeholder="Category" className="input" value={formData.category} onChange={handleChange} required />
        ...
        <input type="text" name="notes" placeholder="Notes" className="input" value={formData.notes} onChange={handleChange} />
      ...
      <div className="overflow-x-auto">
        <table id="business-expense-table" className="min-w-full border border-gray-200 rounded">
          <thead className="bg-blue-50">
            <tr>
              <th className="px-2 py-1">Expense Name</th>
          ...
          <tbody>
            {rows.map((row, idx) => (
              <tr key={idx}>
                <td className="px-2 py-1">{row.expenseName}</td>
...
export default BusinessExpenseTracker; 