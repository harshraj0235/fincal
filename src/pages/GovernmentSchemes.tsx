...
import { Search, Calendar, Users, TrendingUp, BookOpen, Filter, ArrowRight } from 'lucide-react';
...
const GovernmentSchemes: React.FC = () => {
  ...
  const categoryCounts = useMemo(() => {
    ...
    schemeCategories.forEach(cat => {
      counts[cat.id] = governmentSchemes.filter(scheme => scheme.category === cat.name).length;
    });
    return counts;
  }, []);
  ...
  const filteredSchemes = useMemo(() => {
    return governmentSchemes.filter(scheme => {
      const matchesSearch =
        ...
        scheme.keywords.some(keyword => keyword.toLowerCase().includes(searchTerm.toLowerCase()));
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <div className="bg-white p-4 rounded-lg shadow-sm border">
            ...
            <p className="text-2xl font-bold text-green-600">{governmentSchemes.length}</p>
          </div>
          <div className="bg-white p-4 rounded-lg shadow-sm border">
            <div className="flex items-center gap-2">
            ...
            <p className="text-2xl font-bold text-blue-600">{schemeCategories.length}</p>
          </div>
          <div className="bg-white p-4 rounded-lg shadow-sm border">
            <div className="flex items-center gap-2">
        ...
        <div className="mb-8">
            ...
            <div className="flex items-center gap-4">
              ...
              >
                ...
                {statusOptions.map(option => (
                  <option key={option.value} value={option.value}>{option.labelHindi}</option>
                ))}
              </select>
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-3 mb-6">
            ...
            >
              सभी श्रेणियां ({governmentSchemes.length})
            </button>
            {schemeCategories.map(cat => (
              ...
              >
                ...
                {cat.nameHindi} ({categoryCounts[cat.id]})
              </button>
            ))}
          </div>
        </div>
              ...
              <div className="relative">
                ...
                <div className="absolute top-3 left-3">
                  <div className={`px-2 py-1 rounded-full text-xs font-medium ${
                    ...
                    scheme.status === 'upcoming' ? 'bg-blue-500 text-white' :
                    scheme.status === 'past' ? 'bg-gray-500 text-white' :
                    'bg-purple-500 text-white'
                  }`}>
                    {scheme.status === 'active' ? 'सक्रिय' :
                     scheme.status === 'upcoming' ? 'आगामी' :
                     scheme.status === 'past' ? 'पुरानी' : 'भविष्य'}
                  </div>
                </div>
              ...
              <div className="p-6">
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  ...
                  {scheme.targetAudience.slice(0, 1).map(audience => (
                    <span key={audience} className="bg-blue-100 text-blue-700 px-2 py-1 rounded text-xs">
                      {audience}
                    </span>
                  ))}
                </div>
