...
// If there are bullet points starting with "* " in paragraphs, let's just make them <li> inside <ul>?
// This might be too complex for simple regex. The user only mentioned `**` and `#`.
// The regex `\*\*([^*]+)\*\*` already safely ignores single `*`.
